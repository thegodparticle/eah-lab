---
# Display name
title: Niyazi Ömer Arslan, PhD

# Name pronunciation (optional)
# name_pronunciation: Ni-yah-zee Ars-lan

# Full name (for SEO)
first_name: Niyazi
last_name: Arslan

# Is this the primary user of the site?
superuser: true

# Role/Position
role: Assistant Professor

# Organizations/Affiliations
organizations:
  - name: University of South Alabama
    url: 'https://www.southalabama.edu/'

# Short bio (displayed in user profile at bottom of posts)
bio: My research focuses on psychoacoustics, auditory neuroscience, and cochlear implants.

# Research Interests
interests:
  - Psychoacoustics
  - Auditory Neuroscience
  - Cochlear Implants

# Education
education:
  courses:
    - course: PhD in Speech and Hearing Science
      institution: Arizona State University
      year: 2025
    - course: MSc in Audiology and Speech Disorders
      institution: Marmara University
      year: 2019
    - course: BSc in Audiology
      institution: Bezmialem Vakif University
      year: 2017

# Social/Academic Links
# For email, use 'mailto:your-email@southalabama.edu'
social:
  - icon: envelope
    icon_pack: fas
    link: mailto:niyaziomerarslan@gmail.com
  - icon: google-scholar
    icon_pack: ai
    link: https://scholar.google.com/citations?user=cTzzqvMAAAAJ&hl=en
    # Update this with your actual ID later
#  - icon: github
#    icon_pack: fab
#    link: https://github.com/thegodparticle
  - icon: twitter
    icon_pack: fab
    link: https://twitter.com/niyazioarslan
    label: Follow on Twitter
  - icon: linkedin
    icon_pack: fab
    link: https://www.linkedin.com/in/niyazi-arslan/
    label: Connect on LinkedIn

# Link to a Widget Page (e.g., your personal website if separate)
# url_code: ""
# url_dataset: ""
# url_project: ""
# url_source: ""
# url_video: ""

# User groups
user_groups:
  - Faculty
---

Niyazi Ömer Arslan is an Assistant Professor in the Department of Speech Pathology and Audiology at the University of South Alabama and the director of the **Electric & Acoustic Hearing Lab**.

His research investigates the intersection of **psychoacoustics** and **auditory neuroscience**, with a primary focus on improving hearing outcomes for cochlear implant recipients. He is particularly interested in how neural health influences the efficacy of electrical field shaping techniques.

Before joining South Alabama, Dr. Arslan completed his Ph.D. at Arizona State University, where he studied the effects of neural health on electrical stimulation. He also holds an M.Sc. from Marmara University and a B.Sc. from Bezmialem Vakif University in Istanbul.
